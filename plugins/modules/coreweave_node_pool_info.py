...
"""Ansible module: coreweave_node_pool_info."""
...
short_description: Retrieve node pool information
description:
    - Retrieve details about node pools.
...
options:
    pool_name:
        description: ID of a specific node pool to retrieve.
        type: str
    name:
...
def main():
    ...
    if resource_id:
        result = client.get("node_pool", resource_id)
        resources = [result] if result else []
    else:
        resources = client.list("node_pool", module.params)

    module.exit_json(changed=False, node_pools=resources)
