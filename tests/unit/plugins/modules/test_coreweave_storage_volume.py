"""Unit tests for stevefulme1.coreweave.coreweave_storage_volume module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.coreweave.plugins.modules.coreweave_storage_volume"
...
def mock_api_client():
    ...
    client.create.return_value = {"id": "res-123", "name": "test-storage_volume"}
...
def existing_resource():
    """Return a dict representing an existing storage_volume."""
    return {
        "volume_id": "res-123",
        "name": "test-storage_volume",
        "state": "active",
    }


class TestCreateStorageVolume:
    """Tests for creating a storage_volume."""
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        params = {
            "name": "full-storage_volume",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("storage_volume", params)
        mock_api_client.create.assert_called_once_with("storage_volume", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("storage_volume", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "storage_volume": {}}
        else:
            result = mock_api_client.create("storage_volume", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateStorageVolume:
    """Tests for updating a storage_volume."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("storage_volume", "res-123", {"name": "updated"})
        assert result["name"] == "test-storage_volume-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("storage_volume", "bad-id", {})


class TestDeleteStorageVolume:
    """Tests for deleting a storage_volume."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("storage_volume", "res-123")
        mock_api_client.delete.assert_called_once_with("storage_volume", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("storage_volume", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("storage_volume", "res-123")


class TestGetStorageVolume:
    """Tests for getting a storage_volume."""
    ...
    def test_get_nonexistent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        result = mock_api_client.get("storage_volume", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("storage_volume", "res-123")


class TestListStorageVolume:
    """Tests for listing storage_volume resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"volume_id": "1", "name": "first"},
            {"volume_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("storage_volume")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"volume_id": "1", "name": "match"}]
        result = mock_api_client.list("storage_volume", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyStorageVolume:
    """Tests for idempotent behavior of storage_volume."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("storage_volume", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("storage_volume", "missing") is not None
        assert exists is False


class TestErrorHandlingStorageVolume:
    """Tests for error handling in storage_volume."""
